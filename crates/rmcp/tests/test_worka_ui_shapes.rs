...
fn worka_ui_serialization_matches_genui_shapes() {
    let doc = render(Column::new(Children::items(vec![
        ...
        .alignment("center")
        .into(),
        List::vertical(Children::items(vec![
            Text::new("one").into(),
            Text::new("two").into(),
        ]))
        .alignment("start")
        ...
        .into(),
        Slider::new(NumberValue::path("/rating"))
            .min(0.0)
            .max(10.0)
            .into(),
        ...
        Timeline::new(Children::items(vec![
            ...
            TimelineLane::new(
                "lane-1",
                Children::items(vec![TimelineItem::new("item-3").into()]),
            )
            .title("Lane A")
            .into(),
        ]))
    ...
    for kind in [
        ...
        assert!(
            components
                .iter()
                .any(|c| c["component"].get(kind).is_some()),
    ...
    let text_field = components
        ...
        .expect("TextField");
    assert_eq!(
        text_field["component"]["TextField"]["text"],
        json!({ "path": "/email" })
    );
...
fn surface_emits_surface_update_and_begin_rendering() {
    ...
    assert_eq!(
        messages[0]["surfaceUpdate"]["surfaceId"],
        json!("surface-1")
    );
    assert_eq!(
